...
func ParseLogLevel(level string) (logrusLevel logrus.Level) {
	return SwitchLogLevel(log.ParseLevel(level))
}

func SwitchLogLevel(level log.Level) (logrusLevel logrus.Level) {
	switch level {
	case log.LevelDebug:
		logrusLevel = logrus.DebugLevel
	case log.LevelInfo:
